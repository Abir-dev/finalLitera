...
router.get("/referral/me", protect, async (req, res) => {
  try {
    ...
    const inviteLink = `${apiBase}${signupPath}?ref=${encodeURIComponent(user.referralCode)}`;
...
router.post("/referral/regenerate", protect, async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ status: "error", message: "User not found" });
    user.referralCode = undefined; // will be generated by pre-save hook
    await user.save();
    res.status(200).json({ status: "success", data: { referralCode: user.referralCode } });
  } catch (error) {
