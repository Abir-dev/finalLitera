  ...
  getUserStats: async () => {
    try {
      ...
      const stats = {
        ...
        completedCourses:
          user.enrolledCourses?.filter((course) => course.progress === 100)
            .length || 0,
        certificates: user.certificates?.length || 0,
        totalStudyTime:
          user.enrolledCourses?.reduce((total, course) => {
  ...
  getEnrolledCourses: async (filters = {}) => {
    try {
      ...
      try {
        ...
        // Remove items without populated course
        enrollments = enrollments.filter((e) => e && e.course);
        ...
        if (filters.category && filters.category !== "all") {
          enrollments = enrollments.filter(
            (enrollment) => enrollment.course?.category === filters.category
          );
        }

        if (filters.level && filters.level !== "all") {
          enrollments = enrollments.filter(
            (enrollment) => enrollment.course?.level === filters.level
          );
        }
        ...
        if (filters.progress) {
          ...
          switch (filters.progress) {
            case "not-started":
              enrollments = enrollments.filter(
                (enrollment) => getProgressValue(enrollment) === 0
              );
              break;
            case "in-progress":
            ...
            case "completed":
              enrollments = enrollments.filter(
                (enrollment) => getProgressValue(enrollment) === 100
              );
              break;
          }
        }
        ...
        if (filters.sort) {
          switch (filters.sort) {
            case "title-asc":
              enrollments.sort(
                (a, b) => a.course?.title?.localeCompare(b.course?.title) || 0
              );
              break;
            case "title-desc":
              enrollments.sort(
                (a, b) => b.course?.title?.localeCompare(a.course?.title) || 0
              );
              break;
            case "progress-asc":
        ...
        console.log(
          "Enrollment collection not available, falling back to User model:",
        ...
        // Remove items without populated course
        enrolledCourses = enrolledCourses.filter((c) => c && c.course);
