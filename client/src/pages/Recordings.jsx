...
import { Play, Clock, Users, Calendar, Star, BookOpen, Download, Share2, Eye, Heart, MessageCircle, Filter, Search, TrendingUp, Award } from "lucide-react";
  ...
  const recordingTemplates = [
    {
      ...
      description: "Comprehensive introduction to machine learning algorithms and neural networks with hands-on coding examples.",
    ...
    },
    {
      ...
      description: "Deep dive into neural network architectures and backpropagation with visual explanations.",
    ...
    },
    {
      ...
      description: "Complete data analysis workflow using Python, Pandas, and visualization libraries.",
    ...
    },
    {
      ...
      description: "Deploy and scale applications using AWS, Docker, and Kubernetes.",
...
function RecordingCard({ recording }) {
  ...
  return (
    <div className="card-premium overflow-hidden group hover:scale-105 transition-all duration-300">
      ...
      <div className="relative h-48 bg-gradient-to-br from-bg-secondary to-bg-primary overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-16 h-16 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center group-hover:bg-white group-hover:scale-110 transition-all duration-300 shadow-lg">
            <Play size={24} className="ml-1" style={{ color: 'var(--brand)' }} />
          </div>
        </div>
      ...
      <div className="p-6">
        {/* Category */}
        <div className="text-xs font-semibold mb-2 px-2 py-1 rounded-full inline-block" style={{ background: 'var(--brand)10', color: 'var(--brand)' }}>
          {recording.category}
        </div>
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>By {recording.teacher}</span>
        </div>
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t" style={{ borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-3">
            <button className="flex items-center gap-1 text-xs hover:text-brand transition-colors" style={{ color: 'var(--text-muted)' }}>
              <Heart size={14} />
...
export default function RecordingsPage() {
  ...
  const categories = useMemo(() => {
    const cats = [...new Set(liveClasses.map(recording => recording.category))];
    return ['all', ...cats];
  }, []);
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="heading-1 text-3xl md:text-4xl" style={{ color: 'var(--text-primary)' }}>
            Recording Classes
          </h1>
        </div>
        <p className="text-lg" style={{ color: 'var(--text-secondary)' }}>
          Access premium recorded sessions from industry experts and learn at your own pace
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="card-premium p-6 text-center">
          ...
          <div className="text-2xl font-bold mb-1" style={{ color: 'var(--text-primary)' }}>{liveClasses.length}</div>
          <div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Total Recordings</div>
        </div>
        ...
        <div className="card-premium p-6 text-center">
          ...
          <div className="text-2xl font-bold mb-1" style={{ color: 'var(--text-primary)' }}>{liveClasses.filter(r => r.isPremium).length}</div>
          <div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Premium Content</div>
        </div>
        ...
        <div className="card-premium p-6 text-center">
          ...
          <div className="text-2xl font-bold mb-1" style={{ color: 'var(--text-primary)' }}>{liveClasses.filter(r => r.isTrending).length}</div>
          <div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Trending Now</div>
        </div>
        ...
        <div className="card-premium p-6 text-center">
          ...
          <div className="text-2xl font-bold mb-1" style={{ color: 'var(--text-primary)' }}>{liveClasses.reduce((sum, r) => sum + r.views, 0).toLocaleString()}</div>
      ...
      {filteredRecordings.length === 0 && (
        <div className="card-premium p-12 text-center">
          <div className="w-16 h-16 rounded-xl flex items-center justify-center mx-auto mb-4" style={{ background: 'linear-gradient(135deg, var(--brand)20, var(--brand)10)', border: '1px solid var(--brand)30' }}>
            <Search size={32} style={{ color: 'var(--brand)' }} />
          </div>
          <h3 className="heading-3 text-xl mb-2" style={{ color: 'var(--text-primary)' }}>No recordings found</h3>
